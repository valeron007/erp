...
class Tool < ActiveRecord::Base
  ...
  has_many :work_type_tools, inverse_of: :tool

  has_and_belongs_to_many :d_names, join_table: "d_names_tools"
  accepts_nested_attributes_for :d_names, reject_if: :all_blank, allow_destroy: true

  before_save :update_price

  default_scope { order('LOWER(tools.name) ASC') }

  def self.search(query)
    joins("LEFT JOIN d_names_tools ON d_names_tools.tool_id = tools.id LEFT JOIN d_names ON d_names_tools.d_name_id = d_names.id")
    .where("LOWER(tools.name) LIKE :q OR LOWER(tools.tags) LIKE :q OR LOWER(d_names.name) LIKE :q", q: "%#{query.downcase}%")
  end

  def to_s
    "#{name_with_aliases} (#{number_to_currency(price_per_unit)})"
  end
  ...
  def price_to_s
    "#{number_to_currency(price_per_unit)}"
  end
