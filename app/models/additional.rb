...
class Additional < ActiveRecord::Base
  ...
  has_many :work_type_additionals, inverse_of: :additional

  has_and_belongs_to_many :d_names, join_table: "d_names_additionals"
  accepts_nested_attributes_for :d_names, reject_if: :all_blank, allow_destroy: true

  before_save :update_price

  default_scope { order('LOWER(additionals.name) ASC') }

  def self.search(query)
    joins("LEFT JOIN d_names_additionals ON d_names_additionals.additional_id = additionals.id LEFT JOIN d_names ON d_names_additionals.d_name_id = d_names.id")
    .where("LOWER(additionals.name) LIKE :q OR LOWER(additionals.tags) LIKE :q OR LOWER(d_names.name) LIKE :q", q: "%#{query.downcase}%")
  end
