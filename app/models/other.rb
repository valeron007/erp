...
class Other < ActiveRecord::Base
  ...
  has_many :work_type_others, inverse_of: :other

  has_and_belongs_to_many :d_names, join_table: "d_names_others"
  accepts_nested_attributes_for :d_names, reject_if: :all_blank, allow_destroy: true

  before_save :update_price

  default_scope { order('LOWER(others.name) ASC') }
  ...
  def self.search(query)
    joins("LEFT JOIN d_names_others ON d_names_others.other_id = others.id LEFT JOIN d_names ON d_names_others.d_name_id = d_names.id")
    .where("LOWER(others.name) LIKE :q OR LOWER(others.tags) LIKE :q OR LOWER(d_names.name) LIKE :q", q: "%#{query.downcase}%").distinct
  end
